...
public class ThreadTask implements IThreadTask, IThreadTaskObserver {
  ...
  public void breakTask() {
    ...
    if (mTaskWrapper.isSupportBP()) {
      final long currentTemp = mRangeProgress;
      updateState(ITaskManager.STATE_STOP, null);
   ...
   * 发送状态给状态处理器
   *
   * @param state {@link ITaskManager#STATE_STOP}..
   * @param bundle 而外数据
   */
  @Override
  public synchronized void updateState(int state, Bundle bundle) {
    ...
    msg.setData(bundle);
    bundle.putString(ITaskManager.DATA_THREAD_NAME, getThreadName());
    bundle.putLong(ITaskManager.DATA_THREAD_LOCATION, mRangeProgress);
    msg.what = state;
  ...
  private void sendM3U8Info(int state, Message msg) {
    ...
    if (state != ITaskManager.STATE_UPDATE_PROGRESS) {
      msg.obj = this;
    }
    if ((state == ITaskManager.STATE_COMPLETE || state == ITaskManager.STATE_FAIL)) {
  ...
  @Override public synchronized void updateCompleteState() {
    ...
    sendRunningState();
    updateState(ITaskManager.STATE_COMPLETE, null);
  }
  ...
  private void sendRunningState() {
    ...
    b.putString(ITaskManager.DATA_THREAD_NAME, getThreadName());
    b.putLong(ITaskManager.DATA_ADD_LEN, mRangeProgress - mLastRangeProgress);
    msg.what = ITaskManager.STATE_RUNNING;
  ...
  public void cancel() {
    isCancel = true;
    updateState(ITaskManager.STATE_CANCEL, null);
    ALog.d(TAG,
    ...
    if (mConfig.isBlock) {
      ...
      if (!temp.exists()) {
        ...
        if (blockFileLen > threadRect) {
          ...
          mRecord.isComplete = false;
          updateState(ITaskManager.STATE_UPDATE_PROGRESS, null);
          ALog.i(TAG,
  ...
  private void sendFailMsg(AriaException e, boolean needRetry) {
    ...
    updateState(ITaskManager.STATE_FAIL, b);
  }
